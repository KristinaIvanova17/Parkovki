...
def check_adr(message):
    with open('./data/users.json', encoding='utf-8') as jsonfile:
        ...
        result = dadata.clean(
            "address", f"{data[str_id]['select_city']} {message.text}")

        pr_result = result['result']
        pr_region = result['region']
        pr_street = result['street']
        pr_litter = result['block'] if result['block'] != None else ''
        pr_house = result['house'] if result['house'] != None else '' + pr_litter
        pr_geo_lat = result['geo_lat']
        pr_geo_lon = result['geo_lon']

        if pr_region != data[str_id]['select_city']:
            print(f"~~ {data[str_id]['first_name']} ввел '{message.text}' - Неверный адрес")
            return bot.reply_to(
        ...
        if pr_geo_lat != None and pr_geo_lon != None:
            bot.send_location(
                message.chat.id, latitude=pr_geo_lat, longitude=pr_geo_lon)
        print(f"~~ {data[str_id]['first_name']} ввел '{message.text}' - Верный адрес")